...
export interface DayCellState {
  ...
  setDragOver: (
    fn: string | null | ((v: string | null) => string | null),
  ) => void;
...
export function DayPartToggle({
  ...
  state: DayCellState;
  /** Chips only (no "Show" label / border), for embedding in a toolbar. */
  compact?: boolean;
  className?: string;
}) {
  return (
    ...
    >
      ...
      {(
        [
          ["day", compact ? "Day" : "Day (before 4pm)", state.showDay],
          [
            "evening",
            compact ? "Evening" : "Evening (4pm on)",
            state.showEvening,
          ],
        ] as const
      ).map(([part, label, on]) => (
...
export function DayCell({
  ...
  const dayPart = showDay
    ? dayShifts.filter((s) => s.startMin < DIVIDER_MIN)
    : [];
  const eveningPart = showEvening
    ? dayShifts.filter((s) => s.startMin >= DIVIDER_MIN)
    : [];
  const chip = (s: Shift) => {
    ...
    return (
      <button
        ...
        style={
          open ? undefined : { backgroundColor: `${shiftColor(s, emp)}22` }
        }
      >
        {!open && (
          <span
    ...
    >
      ...
      <div className="mb-1 flex items-center justify-between px-0.5">
        ...
        <span className="flex items-center gap-1 text-[10px] text-slate-400">
          {dayShifts.length > 0 && (
            <span
              title={`${dayCount} before 4pm · ${eveCount} from 4pm${openCount ? ` · ${openCount} open` : ""}`}
            >
