...
interface Props extends DayCellCallbacks {
  ...
  stacked?: boolean;
  /** Externally owned cell state (the parent then renders the day/evening toggle itself). */
  state?: DayCellState;
}
...
export function DayList({
  ...
  const own = useDayCellState();
  const state = external ?? own;
  return (
    <div className="card overflow-hidden">
      {!external && <DayPartToggle state={state} />}
      <div
        className={
          stacked
