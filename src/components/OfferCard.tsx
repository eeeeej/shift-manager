...
export function OfferCard({ offer }: { offer: ShiftOffer }) {
  const { me, isAdmin, shiftById, employeeById, claimOffer, cancelOffer } = useData()
  ...
  const target = offer.targetEmployeeId ? employeeById(offer.targetEmployeeId) : undefined
    ...
    <div className={`card p-4 ${offer.status !== 'open' ? 'opacity-75' : ''}`}>
      ...
      {(canClaim || canCancel || isAdmin) && offer.status === 'open' && (
        <div className="mt-3 flex flex-wrap gap-2 [&>button]:min-h-11 [&>button]:flex-1 sm:[&>button]:min-h-0 sm:[&>button]:flex-none">
          ...
          {isAdmin && (
            <button className="btn-secondary" disabled={busy} onClick={() => setReassign(true)}>
              Reassign
            </button>
          )}
          {canCancel && (
            <button className="btn-ghost" disabled={busy} onClick={() => act(() => cancelOffer(offer.id))}>
      ...
      {reassign && <ReassignModal shift={shift} onClose={() => setReassign(false)} />}
    </div>
  )
}
