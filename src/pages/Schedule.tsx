...
import { addDays, dateRange, formatDateLong, formatDateShort, fromDateKey, startOfWeek, todayKey } from '../utils/time'
...
export function Schedule() {
  ...
  const range = useMemo(() => {
    ...
    return dateRange(s, Math.round((fromDateKey(end).getTime() - fromDateKey(s).getTime()) / 86400000) + 1)
  }, [start, days, isMonth, months, isWeekFeed, weeks, isDesktop, visibleMonth])
  /** The week/day run the header, Copy week and + Shift act on (feed views scroll, so use the visible one). */
  const focusRange = useMemo(() => (isWeekFeed ? dateRange(visibleWeek, 7) : range), [isWeekFeed, visibleWeek, range])
  ...
  const showMonth = (m: string) => (isDesktop ? jumpToMonth(m) : setVisibleMonth(m))
  const step = (dir: 1 | -1) => {
  ...
  useEffect(() => {
    ...
    const update = () => {
      const limit = (controls.current?.getBoundingClientRect().bottom ?? 0) + 12
      let current: string | null = null
      for (const el of document.querySelectorAll<HTMLElement>('[data-date]')) {
        if (el.getBoundingClientRect().top <= limit) current = el.dataset.date ?? null
        else break
      }
  ...
  // Phones: swipe left/right to move to the next/previous day, run or month.
  ...
  const onTouchEnd = (e: TouchEvent) => {
    ...
    if (Math.abs(dx) > threshold && Math.abs(dx) > 2.5 * Math.abs(dy)) step(dx < 0 ? 1 : -1)
  }
  ...
  return (
    <div>
      ...
      >
        <PageHeader
          title={topDay ? formatDateShort(topDay) : 'Schedule'}
          subtitle={title}
