...
export function Schedule() {
  ...
  const range = useMemo(() => dateRange(start, days), [start, days])
  ...
  const offeredShiftIds = useMemo(() => new Set(offers.filter((o) => o.status === 'open').map((o) => o.shiftId)), [offers])

  const step = (dir: 1 | -1) => setStart(addDays(start, dir * days))
  const goToday = () => setStart(days === 7 ? startOfWeek(todayKey()) : todayKey())
  ...
  const title =
    days === 1
      ...
      : `${first.toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} – ${last.toLocaleDateString(undefined, {
          month: 'short',
          day: 'numeric',
          year: 'numeric',
        })}`
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={
          isAdmin && (
            <button className="btn-primary" onClick={() => setDraft({ date: range[0], position: position || 'Server' })}>
              <Plus size={16} /> Shift
            </button>
          )
        }
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        ...
        <select className="input w-auto" value={position} onChange={(e) => setPosition(e.target.value as Position | '')}>
      ...
      {isDesktop ? (
        <TimelineGrid
          ...
          onSlotClick={isAdmin ? (date, startMin) => setDraft({ date, startMin, endMin: Math.min(startMin + 300, 23 * 60), position: position || 'Server' }) : undefined}
        />
      ) : (
        <div className="space-y-4">
          {range.map((d) => {
            const dayShifts = visible.filter((s) => s.date === d).sort((a, b) => a.startMin - b.startMin || a.endMin - b.endMin)
            const isToday = d === todayKey()
            return (
              <section key={d}>
                <h3 className={`mb-1.5 flex items-center gap-2 text-sm font-semibold ${isToday ? 'text-slate-900' : 'text-slate-600'}`}>
                  {formatDateLong(d)}
                  ...
                  <div className="space-y-1.5">
                    {dayShifts.map((s) => (
                      <ShiftRow
                        ...
                        onClick={() => openShift(s)}
                        trailing={offeredShiftIds.has(s.id) ? <span className="chip bg-amber-100 text-amber-800">trade</span> : undefined}
                      />
                    ))}
                  </div>
                )}
